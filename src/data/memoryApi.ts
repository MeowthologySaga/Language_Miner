...
import { translateTextsWithGoogle } from "../shared/googleTranslation";
import { buildGoogleTranslationRequest } from "../shared/googleTranslationRequest";
...
export function createMemoryApi(): LocalEnglishMinerApi {
  ...
  const runTranslationJob = async <T>(
    requestId: string | undefined,
    run: (signal: AbortSignal) => Promise<T>
  ): Promise<T> => {
    const normalizedRequestId = normalizeMemoryTranslationRequestId(requestId);
    if (normalizedRequestId && translationControllers.has(normalizedRequestId)) {
      throw new Error(`Translation request id is already active: ${normalizedRequestId}`);
    }
    const controller = new AbortController();
    if (normalizedRequestId) {
      translationControllers.set(normalizedRequestId, controller);
    }
    try {
      ...
      if (
        normalizedRequestId &&
        translationControllers.get(normalizedRequestId) === controller
      ) {
        translationControllers.delete(normalizedRequestId);
      }
    }
  };
  ...
  return {
    ...
    profiles: {
      ...
      async getDataSummary(profileId = DEFAULT_PROFILE_ID) {
        const normalized = normalizeProfileId(profileId);
        const cards = readJson<StudyCard[]>(CARDS_KEY, []).filter((card) =>
          cardBelongsToProfile(card, normalized)
        );
        const now = Date.now();
        return {
          profileId: normalized,
          cardCount: cards.length,
          dueCardCount: cards.filter((card) => new Date(card.srs.dueAt).getTime() <= now).length,
          translationCacheCount: readJson<TranslationCacheEntry[]>(TRANSLATION_CACHE_KEY, []).filter(
            (entry) => normalizeProfileId(entry.profileId) === normalized
          ).length,
          exportRecordCount: readJson<BilingualExportHistoryRecord[]>(EXPORT_RECORDS_KEY, []).filter(
            (entry) => normalizeProfileId(entry.profileId) === normalized
          ).length
        };
      },
      async deleteData(profileId) {
        const normalized = normalizeProfileId(profileId);
        ...
        const cache = readJson<TranslationCacheEntry[]>(TRANSLATION_CACHE_KEY, []);
        const exports = readJson<BilingualExportHistoryRecord[]>(EXPORT_RECORDS_KEY, []);
        const missionEvents = readJson<LearningMissionEvent[]>(MISSION_EVENTS_KEY, []);
        const transactions = readJson<DiamondTransaction[]>(DIAMOND_TRANSACTIONS_KEY, []);
        const logs = readJson<LifeLog[]>(LIFE_LOGS_KEY, []);
        const nextLogs = logs.map((log) => ({
          ...log,
          metadata: removeLifeLogProfileProgress(log.metadata, normalized)
        }));
        const lifeLogsUpdated = logs.filter(
          (log, index) => JSON.stringify(log.metadata) !== JSON.stringify(nextLogs[index]?.metadata)
        ).length;
        writeJson(CARDS_KEY, cards.filter((card) => !cardBelongsToProfile(card, normalized)));
        writeJson(TRANSLATION_CACHE_KEY, cache.filter((entry) => normalizeProfileId(entry.profileId) !== normalized));
        writeJson(EXPORT_RECORDS_KEY, exports.filter((entry) => normalizeProfileId(entry.profileId) !== normalized));
        writeJson(MISSION_EVENTS_KEY, missionEvents.filter((entry) => normalizeProfileId(entry.profileId) !== normalized));
        writeJson(DIAMOND_TRANSACTIONS_KEY, transactions.map((entry) =>
          normalizeProfileId(entry.profileId) === normalized ? { ...entry, profileId: undefined } : entry
        ));
        writeJson(LIFE_LOGS_KEY, nextLogs);
        return {
          profileId: normalized,
          cards: cards.filter((card) => cardBelongsToProfile(card, normalized)).length,
          translationCacheEntries: cache.filter((entry) => normalizeProfileId(entry.profileId) === normalized).length,
          exportRecords: exports.filter((entry) => normalizeProfileId(entry.profileId) === normalized).length,
          missionEvents: missionEvents.filter((entry) => normalizeProfileId(entry.profileId) === normalized).length,
          walletTransactions: transactions.filter((entry) => normalizeProfileId(entry.profileId) === normalized).length,
          lifeLogsUpdated
        };
      }
    },
    cards: {
      ...
      async listPage(profileId = DEFAULT_PROFILE_ID, offset = 0, limit = 100) {
        const items = readJson<StudyCard[]>(CARDS_KEY, [])
          .map((card) => normalizeCardDeck(card))
          .filter((card) => cardBelongsToProfile(card, profileId))
      ...
      async save(card, profileId = DEFAULT_PROFILE_ID) {
        ...
        if (!existing && !isDefaultSampleCardId(saved.id)) {
    ...
    wallet: {
      ...
      async lookupSpend(input) {
        return lookupStoredDiamondSpend(input);
      },
      async spend(input) {
        return spendStoredDiamonds(input);
      }
    },
    missions: {
      async getToday() {
    ...
    lifeLogs: {
      ...
      async listPage(offset = 0, limit = 100) {
        const logs = readJson<LifeLog[]>(LIFE_LOGS_KEY, []).sort((left, right) =>
          right.createdAt.localeCompare(left.createdAt)
        );
        const normalizedOffset = Math.max(0, Math.floor(offset));
        const normalizedLimit = Math.min(500, Math.max(1, Math.floor(limit)));
        return {
          items: logs.slice(normalizedOffset, normalizedOffset + normalizedLimit),
          total: logs.length,
          offset: normalizedOffset,
          limit: normalizedLimit
        };
    ...
    translations: {
      async cancel(requestId) {
        const normalizedRequestId = normalizeMemoryTranslationRequestId(requestId, true);
        const controller = translationControllers.get(normalizedRequestId);
        if (!controller) {
          return false;
        }
      ...
      async ensureOllamaRunning(baseUrl) {
        return {
          baseUrl: baseUrl?.trim() || "http://127.0.0.1:11434",
          status: "unsupported_platform"
        } satisfies EnsureOllamaRuntimeResult;
      },
      ...
      async translate(input) {
        return runTranslationJob(input.requestId, async (signal) => {
          const controlledInput = { ...input, signal, requestBudget: undefined };
          ...
          if (input.providerName === "google") {
            const translatedText = await translateWithGoogle(controlledInput);
          ...
          if (input.providerName === "gemini") {
            const geminiResult = await translateTextWithGemini(controlledInput);
            throwIfTranslationAborted(signal);
          ...
          const translatedText = await translateWithLocalOllama(controlledInput);
          throwIfTranslationAborted(signal);
          const saved = saveCachedTranslation(input, translatedText);
          return translationResultFromEntry(
            saved,
      ...
      async translatePdfSegments(input) {
        return runTranslationJob(input.requestId, (signal) =>
          translatePdfSegments({ ...input, signal })
        );
      }
    },
    tts: {
...
function normalizeMemoryTranslationRequestId(requestId: unknown, required: true): string;
function normalizeMemoryTranslationRequestId(
  requestId: unknown,
  required?: false
): string | undefined;
function normalizeMemoryTranslationRequestId(
  ...
  if (requestId === undefined || requestId === null || requestId === "") {
    if (required) throw new Error("Translation request id is required.");
    return undefined;
  }
  if (
    ...
    throw new Error("Translation request id has an invalid format.");
  }
  return requestId;
}
...
function lookupStoredDiamondSpend(input: DiamondSpendRequest): DiamondSpendLookupResult {
  const wallet = getStoredDiamondWallet();
  const amount = Math.floor(Number(input.amount));
  const idempotencyKey = input.idempotencyKey.trim();
  const reason = input.reason.trim() || "PlayZone diamond spend";
  ...
  if (!Number.isFinite(amount) || amount <= 0) {
    return {
      ok: false,
      code: "invalid_amount",
      message: "Diamond spend amount must be positive.",
      balance: wallet.balance,
      wallet
    };
  }
  const replay = readJson<DiamondTransaction[]>(DIAMOND_TRANSACTIONS_KEY, []).find(
    (entry) => entry.type === "spend" && entry.idempotencyKey === idempotencyKey
  );
  if (!replay) return null;
  if (replay.amount !== amount || replay.reason !== reason || replay.profileId !== profileId) {
    return {
      ok: false,
      code: "idempotency_conflict",
      message: "The idempotency key was already used for a different spend.",
      balance: wallet.balance,
      wallet
    };
  }
...
function spendStoredDiamonds(input: DiamondSpendRequest) {
  const now = new Date().toISOString();
  const wallet = getStoredDiamondWallet();
  const amount = Math.floor(Number(input.amount));
  const idempotencyKey = input.idempotencyKey.trim();
  const existing = lookupStoredDiamondSpend(input);
  ...
  if (wallet.balance < amount) {
    return {
      ...
      message: "There are not enough diamonds.",
      balance: wallet.balance,
      wallet
    };
  }
  ...
  const transaction: DiamondTransaction = {
    ...
    amount,
    balanceAfter: nextWallet.balance,
    reason: input.reason.trim() || "PlayZone diamond spend",
    profileId: normalizeProfileId(input.profileId),
    dateKey: getMissionDateKey(new Date(now)),
    createdAt: now,
    idempotencyKey
  };
  writeJson(DIAMOND_WALLET_KEY, nextWallet);
...
async function testTranslationConnection(
  ...
  try {
    ...
    if (input.providerName === "google") {
      ...
      return { ok: true, code: "connected", providerName: "google" };
    }
    ...
    if (!input.ollamaModel?.trim()) {
      return {
        ...
        code: "model_required",
        providerName: "local",
        baseUrl: input.ollamaBaseUrl?.trim()
      };
    }
    const status = await fetchOllamaModelStatus({
      baseUrl: input.ollamaBaseUrl,
      model: input.ollamaModel ?? ""
    });
    return {
      ok: status.installed,
      code: status.installed ? "connected" : "ollama_model_missing",
      providerName: "local",
      model: status.model,
      ...
      model:
        input.providerName === "local"
          ...
          : input.providerName === "gemini"
            ...
            : undefined,
      baseUrl: input.providerName === "local" ? input.ollamaBaseUrl?.trim() : undefined
    };
  }
}
...
async function translateWithGoogle(input: TranslateTextInput & TranslationRequestControl) {
