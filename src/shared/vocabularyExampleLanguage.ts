...
export function createVocabularyExampleLanguageRules(profile: LearningProfile) {
  ...
  return [
    ...
    `- Do not put ${nativeLabel} translations inside vocabularyItems[].examples; examples are target-language usage examples, not native-language explanations.`,
    "- vocabularyItems[].exampleTranslationsKo must contain Korean translations for examples[] in the same order, one translation per example.",
...
export function createTargetLanguageFallbackExampleTranslationsKo(
  term: string,
  targetLanguageCode: string
) {
  const normalizedTerm = normalizeExampleTerm(term);
  ...
  if (code === "ja") {
    return [
      `"${normalizedTerm}"은 이 문장에서 자연스럽게 쓰입니다.`,
      `그는 "${normalizedTerm}"이라는 표현을 사용했습니다.`,
      `이 장면에서는 "${normalizedTerm}"이 중요한 의미를 가집니다.`
    ];
  }
  if (code === "ko") {
    return createTargetLanguageFallbackExamples(normalizedTerm, code);
  }
  if (code === "en" || !code) {
    return [
      `문장에서 "${normalizedTerm}"이라는 표현을 발견했습니다.`,
      `짧은 답변에서 "${normalizedTerm}"을 써 보세요.`,
      `"${normalizedTerm}"이라는 표현은 말투를 바꿉니다.`
    ];
  }
  return [];
}
