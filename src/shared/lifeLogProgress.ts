...
export function removeLifeLogProfileProgress(
  metadata: LifeLogMetadata | undefined,
  profileId: ProfileId | undefined
  ...
  const normalizedProfileId = normalizeLifeLogProfileId(profileId);
  const processedProfileIds = getLifeLogProcessedProfileIds(metadata).filter(
    (id) => id !== normalizedProfileId
  );
  const nextMetadata = { ...metadata };
  if (processedProfileIds.length > 0) {
    nextMetadata.processedProfileIds = processedProfileIds;
  } else {
    delete nextMetadata.processedProfileIds;
  }
  return nextMetadata;
}
