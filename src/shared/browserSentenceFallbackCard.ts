...
import {
  createTargetLanguageFallbackExampleTranslationsKo,
  createTargetLanguageFallbackExamples,
...
export function createFallbackVocabularyItem(
  ...
  return {
    ...
    colorKey,
    examples,
    exampleTranslationsKo: createFallbackExampleTranslationsKo(
      term,
      examples,
      targetLanguageCode,
      detail.examples,
      detail.exampleTranslationsKo
    )
  };
}

function createFallbackExampleTranslationsKo(
  term: string,
  examples: string[],
  targetLanguageCode?: string,
  detailExamples?: string[],
  detailTranslations?: string[]
) {
  const code = targetLanguageCode || "en";
  const translationByExample = new Map<string, string>();
  detailExamples?.forEach((example, index) => {
    const translation = detailTranslations?.[index]?.trim();
    if (translation) {
      translationByExample.set(normalizeTermKey(example), translation);
    }
  });
  createTargetLanguageFallbackExamples(term, code).forEach((example, index) => {
    const translation = createTargetLanguageFallbackExampleTranslationsKo(term, code)[index];
    if (translation) {
      translationByExample.set(normalizeTermKey(example), translation);
    }
  });
  return examples.map((example) => translationByExample.get(normalizeTermKey(example)) ?? "");
}
...
export function ensureBrowserSentenceSelectedTerms<T extends BrowserSentenceTermCard>(
  ...
  const vocabularyItems = terms.map((term, index) => {
    ...
    const hasExplicitEtymologyField =
      existingItem && Object.prototype.hasOwnProperty.call(existingItem, "etymologyKo");
    return existingItem
        ? {
          ...existingItem,
          ipa: normalizeBrowserVocabularyIpa(existingItem.term || term, existingItem.ipa, fallbackItem.ipa),
          etymologyKo: hasExplicitEtymologyField
            ? existingItem.etymologyKo?.trim() || undefined
            : fallbackItem.etymologyKo,
  ...
  return {
    ...result,
    highlightMappings: sanitizeInputHighlightMappings(result)
  };
}
...
const knownComparisonDetails: Record<string, BrowserSentenceComparison> = {
  ...
  facade: {
    ...
    explanationKo:
      "facade는 건물 정면이라는 전문적인 느낌이 강하고, face는 사람 얼굴이나 사물의 앞면을 더 넓게 가리킵니다. 예: a glass facade / a friendly face."
  },
  facades: {
    kind: "similar",
    title: "facades vs faces",
    explanationKo:
      "facades는 건물 외벽·정면을 말할 때 전문적인 느낌이 있고, faces는 사람 얼굴이나 표면을 더 넓게 가리킵니다. 예: historic facades / familiar faces."
  },
  neglect: {
    kind: "similar",
...
function getFallbackTermDetail(term: string): FallbackTermDetail {
  ...
  return {
    ...
    exampleTranslationsKo: [
      `문장에서 "${term}"이라는 표현을 발견했습니다.`,
      `짧은 답변에서 "${term}"을 써 보세요.`,
      `"${term}"이라는 표현은 말투를 바꿉니다.`
    ]
  };
}
...
const knownTermDetails: Record<string, FallbackTermDetail> = {
  ...
  range: {
    ipa: "/rend?/",
    partOfSpeech: "noun",
    basicMeaningKo: "범위, 거리",
    meaningInContextKo: "스킬이나 공격이 닿는 거리 또는 적용 범위를 말합니다.",
    literalKo: "범위",
    naturalKo: "거리/범위",
    etymologyKo:
      "range는 '줄, 배열, 범위'에서 온 말로, 게임 문맥에서는 공격이나 스킬이 닿는 거리입니다. long/short range는 장거리/단거리, ranged는 '원거리의'라는 형용사로 씁니다.",
    usagePatterns: [
      "Collocation: long range",
      "Collocation: short range",
      "within range",
      "ranged skill"
    ],
    examples: [
      "The sniper's rifle had a long range.",
      "Keep the tools within easy range.",
      "This skill works at short range."
    ],
    exampleTranslationsKo: [
      "저격수의 소총은 사거리가 길었다.",
      "도구들을 쉽게 닿을 수 있는 범위 안에 두세요.",
      "이 스킬은 근거리에서 작동합니다."
    ]
  },
  attention: {
    ...
    exampleTranslationsKo: [
      "도로에 주의를 기울여 주세요.",
      "그녀는 세부 사항에 세심하게 주의를 기울였다.",
      "나는 통화 중에 집중하고 있지 않았다."
    ]
  },
  hibernated: {
    ipa: "/ˈhaɪbərneɪtɪd/",
    ...
    exampleTranslationsKo: [
      "곰은 겨울 내내 겨울잠을 잤다.",
      "내 노트북은 한 시간 뒤에 절전 상태가 되었다.",
      "그는 몇 년 동안 긴 잠에 빠져 있었던 것처럼 보였다."
    ]
  }
};
