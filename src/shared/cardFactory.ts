...
import { normalizeCardTags } from "./cardTags";
import { normalizeReadingSentenceStructure } from "./readingStructure";
...
export function createStudyCardFromGenerated(data: GeneratedCardData): StudyCard {
  ...
  const card = normalizeCardDeck({
    ...data,
    readingStructure:
      data.cardType === "reading" && (data.deckType ?? "input") === "input"
        ? normalizeReadingSentenceStructure(data.readingStructure, data.sourceSentence)
        : data.readingStructure,
    tags: normalizeCardTags(data.tags ?? data.outputStudyGuide?.tags),
    id: data.id ?? randomId(),
