...
export type ProviderName = "mock" | "ollama" | "gemini" | "chatgptWeb";

export type CloudProviderName = "gemini" | "google";

export type CloudProviderConsentRecord = {
  version: 1;
  provider: CloudProviderName;
  acceptedAt: string;
  keyStorage: "safeStorage" | "session";
...
export type LifeMiningCaptureSettings = {
  /** Explicit consent gate. Automatic capture must remain off unless this is true. */
  enabled: boolean;
...
export type PlayZoneLibraryEntrySourceType = "file" | "folder";

export type PlayZoneOfficialDownloadInfo = {
  assetUrl: string;
...
export type PlayZoneOfficialDownloadProgress = {
  requestId: string;
  packId: string;
  state: "downloading" | "verifying" | "installing" | "complete" | "cancelled";
  receivedBytes: number;
  totalBytes: number;
};

export type PlayZoneLibraryEntryStatus =
  | "quarantined"
  | "blocked"
  | "warning"
  | "ready"
...
export type PlayZoneSecurityIssueSeverity = "warning" | "error";

export type PlayZoneSecurityIssue = {
  code: string;
  severity: PlayZoneSecurityIssueSeverity;
  message: string;
  file?: string;
};

export type PlayZoneDiamondAction = {
  id: string;
  amount: number;
  reason: string;
  /** The host always confirms wallet spends. Kept for manifest compatibility. */
  requiresConfirm: true;
  repeatable: boolean;
};

export type PlayZonePermissions = {
  walletSpend: boolean;
  storage: boolean;
  network: boolean;
...
export type PlayZoneSecurityReport = {
...
export type PlayZoneRuntimeAuthorization = {
  cartridgeId: string;
  title: string;
  entryUrl: string;
  status: "ready" | "trusted_official";
  permissions: PlayZonePermissions;
  diamondActions: PlayZoneDiamondAction[];
};

export type PlayZoneLibraryEntry = {
  id: string;
  title: string;
  creator: string;
  version?: string;
  ...
  /** Opaque app-managed snapshot identifier. Present only after explicit installation. */
  ...
  entryUrl?: string;
  thumbnailUrl?: string;
  officialDownload?: PlayZoneOfficialDownloadInfo;
...
export type PlayZoneSaveBackupResult = {
  backedUp: boolean;
  backupPath?: string;
  backedUpAt: string;
};

export type PlayZoneLibraryScanResult = {
  folderPath: string;
  folderName: string;
...
export type DiamondSpendRequest = {
  amount: number;
  reason: string;
  profileId?: ProfileId;
  /** Stable caller-scoped key used to make a spend durable and replay safe. */
  idempotencyKey: string;
};

export type DiamondSpendResult =
  | {
      ok: true;
  ...
  | {
      ok: false;
      code: "insufficient_balance" | "invalid_amount" | "invalid_idempotency_key" | "idempotency_conflict";
      message: string;
      balance: number;
      wallet: DiamondWallet;
    };

/** Existing durable result for a spend key, or null when the spend has not happened yet. */
export type DiamondSpendLookupResult = DiamondSpendResult | null;
...
export type ProfileDataDeleteResult = {
  profileId: ProfileId;
  cards: number;
...
export type ProfileDataSummary = {
  ...
  cardCount: number;
  dueCardCount: number;
  translationCacheCount: number;
  exportRecordCount: number;
};

export type CardPageResult = {
  items: StudyCard[];
  total: number;
  offset: number;
  limit: number;
...
export type SecureSettingsStatus = {
  available: boolean;
...
export type ConfusingComparisonKind = "similar" | "contrast" | "nuance";
...
export type OutputStudySentence = {
  en: string;
  ko: string;
...
export type OutputStudyGuide = {
  templateVersion: "adaptive-v1";
  prototype?: boolean;
  contextKo: string;
  dialogue: OutputStudySentence[];
  keyChunks: OutputStudyChunk[];
  insight: {
    title: string;
    bodyKo: string;
  };
  ...
  alternatives: OutputStudySentence[];
  commonMistake?: {
    wrong?: OutputStudySentence;
    right: OutputStudySentence;
    explanationKo: string;
  };
  miniDrills: OutputStudySentence[];
  tags: string[];
};

export type ReadingSentenceStructure = {
  segments: Array<{
    id: string;
    text: string;
    labelKo: string;
    tone: "subject" | "action" | "object" | "connector";
    groupId: string;
  }>;
  groups: Array<{
    id: string;
...
export type ListeningStudyGuide = {
...
export type StudyCard = {
  ...
  readingStructure?: ReadingSentenceStructure;
  listeningStudyGuide?: ListeningStudyGuide;
  syncMetadata?: {
    conflict?: boolean;
...
export type GenerateReadingCardInput = {
  ...
  generationMode?: "reading" | "listening";
  signal?: AbortSignal;
};
...
export type CharacterChatMode = "native_capture" | "target_practice";

export type CharacterCorrectionMode =
  | "off"
  | "instant"
  | "turn_summary"
  | "session_summary";
...
export type CharacterEmotionImageUrls = Partial<Record<CharacterEmotion, string>>;

export type CharacterExpressionImageUrls = CharacterEmotionImageUrls;
...
export type CharacterPreset = {
  ...
  emotionImageUrls?: CharacterEmotionImageUrls;
  expressionFallbackEmotion?: CharacterEmotion;
  ...
  packMetadata?: {
    ...
    sourceUrl: string;
    license: string;
    requestedPermissions: Array<"remote_images">;
    releaseNotes: string;
...
export type TranslateTextInput = TranslationCacheLookupInput & {
  /** Renderer-generated id used to cancel this in-flight translation job. */
  requestId?: string;
  googleApiKey?: string;
  geminiApiKey?: string;
  geminiModel?: string;
...
export type EnsureOllamaRuntimeResult = {
  baseUrl: string;
  status:
    | "already_running"
    | "started"
...
export type TranslationConnectionStatusCode =
  | "connected"
  | "browser_renderer_only"
  | "api_key_required"
  | "model_required"
  | "ollama_runtime_not_installed"
  | "ollama_runtime_start_failed"
  | "ollama_model_missing"
  | "ollama_server_unreachable"
  | "ollama_model_list_failed"
  | "local_mt_setup_failed"
  | "provider_request_failed";

export type TranslationConnectionTestResult = {
  ok: boolean;
  code: TranslationConnectionStatusCode;
  providerName: TranslationProviderName;
  model?: string;
  baseUrl?: string;
};
