...
export async function testLocalMtSetup(
  ...
  const normalizedModel = normalizeLocalMtModel(model);
  await getLocalMtPipeline(normalizedModel, {
    ...options,
...
async function getLocalMtPipeline(model: string, options: LocalMtRuntimeOptions) {
  ...
  try {
    ...
    if (localMtPipelinePromises.get(cacheKey) === pipelinePromise) {
      localMtPipelinePromises.delete(cacheKey);
    }
    throw caught;
  }
}
