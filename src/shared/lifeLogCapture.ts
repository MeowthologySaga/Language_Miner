...
export function maskSensitiveText(text: string) {
  return normalizeRawLifeLogContent(text)
    ...
    // Prefer a false positive over persisting a secret. Development/test keys are often
    // deliberately short, but they must receive the same protection as production keys.
    .replace(/\bsk(?:-proj)?-[0-9A-Za-z_-]{4,}\b/g, "[token]")
