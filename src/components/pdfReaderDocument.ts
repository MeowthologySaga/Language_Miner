...
export function usePdfPageRenderer({
  ...
  currentPage: number;
  onError: (summary: string, caught: unknown) => void;
  ...
  useEffect(() => {
    ...
    async function renderPage() {
      ...
      onStatus(t("pdfAuthoring.documentRenderer.rendering"));
      try {
        ...
        if (!context) {
          throw new Error(t("pdfAuthoring.documentRenderer.contextUnavailable"));
        }
        ...
        if (!cancelled && (caught as { name?: string }).name !== "RenderingCancelledException") {
          onError(t("pdfAuthoring.documentRenderer.renderFailed"), caught);
          onStatus("");
        }
      }
    }
  ...
  }, [canvasRef, currentPage, onError, onStatus, pdfDocument, t, textLayerRef]);
}
