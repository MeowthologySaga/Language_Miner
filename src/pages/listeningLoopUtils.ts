...
export type ListeningLoopSeedDisplayOptions = {
  dailyTargetSentenceCount?: number;
  dailyUsePartialVideoClips?: boolean;
...
export function localizeListeningLoopSeedDisplay(
  seed: ListeningLoopSeed,
  t: TFunction,
  options: ListeningLoopSeedDisplayOptions = {}
): ListeningLoopSeed {
  const displayCopyKey =
    seed.displayCopyKey ??
    (seed.id.startsWith("daily-routine:")
      ? "dailyRoutine"
      : seed.id.startsWith("transcript:")
        ? "generatedTranscript"
        : undefined);
  const formatNumber = options.formatNumber ?? ((value: number) => String(value));

  if (displayCopyKey === "shortEverydaySample") {
    return {
      ...seed,
      topicLabel: t("listeningLoop.seedCatalog.shortEverydaySample.topicLabel"),
      recommendedReason: t(
        "listeningLoop.seedCatalog.shortEverydaySample.recommendedReason"
      )
    };
  }

  if (displayCopyKey === "generatedTranscript") {
    return {
      ...seed,
      topicLabel: t("listeningLoop.seedCatalog.generatedTranscript.topicLabel"),
      recommendedReason: t(
        "listeningLoop.seedCatalog.generatedTranscript.recommendedReason",
        { formattedCount: formatNumber(seed.segments.length) }
      )
    };
  }

  if (displayCopyKey === "dailyRoutine") {
    const clipCount = getDailyRoutineClipCount(seed);
    const preparedSentenceCount = seed.segments.length;
    const targetSentenceCount =
      options.dailyTargetSentenceCount ?? preparedSentenceCount;
    const sourceChannelName = seed.segments[0]?.sourceChannelName?.trim();
    const channelName =
      clipCount > 1
        ? t("listeningLoop.seedCatalog.dailyRoutine.channelName", {
            formattedCount: formatNumber(clipCount)
          })
        : sourceChannelName || seed.channelName;

    return {
      ...seed,
      title: t("listeningLoop.seedCatalog.dailyRoutine.title"),
      channelName,
      levelLabel: t("listeningLoop.seedCatalog.dailyRoutine.levelLabel"),
      topicLabel: t("listeningLoop.seedCatalog.dailyRoutine.topicLabel"),
      recommendedReason: t(
        options.dailyUsePartialVideoClips
          ? "listeningLoop.seedCatalog.dailyRoutine.partialReason"
          : "listeningLoop.seedCatalog.dailyRoutine.fullReason",
        {
          clipCount: formatNumber(clipCount),
          preparedCount: formatNumber(preparedSentenceCount),
          targetCount: formatNumber(targetSentenceCount)
        }
      )
    };
  }
  ...
  const seed =
    firstSegment && segments.length > 0
      ? {
          ...
          topicLabel: "",
          recommendedReason: "",
          displayCopyKey: "dailyRoutine" as const,
          segments
        }
      : null;
