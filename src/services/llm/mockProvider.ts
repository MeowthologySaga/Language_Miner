...
export class MockProvider implements LLMProvider {
  ...
  async generateReadingCard(input: GenerateReadingCardInput): Promise<GeneratedCardData> {
    ...
    if (
      sentence.toLowerCase().includes("i'm running a little late") ||
      sentence.toLowerCase().includes("i’m running a little late")
    ) {
      const generated: GeneratedCardData = {
        ...
        naturalTranslationKo: "조금 늦을 것 같아요.",
        highlightMappings: [
          {
        ...
        vocabularyItems: [],
        structureNote:
          "be running late는 예정된 시간보다 늦어지고 있는 상황을 자연스럽게 알릴 때 씁니다.",
  ...
  async generateCharacterChatReply(input: GenerateCharacterChatReplyInput): Promise<string> {
    ...
    const baseReply = hint?.terms.length
      ? `${characterName}: I get what you mean. ${recentUserLine ? `When you say "${recentUserLine}", ` : ""}it makes me think of "${hint.terms[0]}" in a pretty natural way. Tell me the part that actually matters to you.`
      : `${characterName}: I hear you. Start from the messy part, not the polished version. What happened?`;
    const emotionTag = `[EMOTION]${inferMockCharacterEmotion(recentUserLine)}[/EMOTION]`;
    if (input.chatMode === "target_practice" && input.correctionMode && input.correctionMode !== "off") {
      return [
        baseReply,
        "",
        "[CORRECTION_KO]",
        "방금 문장은 더 자연스럽게 다듬을 수 있어요.",
        "[/CORRECTION_KO]",
        "[SUGGESTED_TARGET]",
        recentUserLine || "Tell me what happened.",
        "[/SUGGESTED_TARGET]",
        emotionTag
      ].join("\n");
    }
    return [baseReply, "", emotionTag].join("\n");
  }
}
...
function inferMockCharacterEmotion(userLine: string) {
  const normalized = userLine.toLowerCase();
  if (
  ...
  if (
    ...
    normalized.includes("tell you")
  ) {
    return "listening";
  }
  if (/[!]/.test(userLine) || normalized.includes("wow") || normalized.includes("really")) {
    return "surprised";
  }
  if (normalized.includes("hard") || normalized.includes("sad")) {
    return "concerned";
  }
  if (normalized.includes("how") || normalized.includes("why") || normalized.includes("think")) {
    return "thinking";
  }
  if (normalized.includes("did it") || normalized.includes("done") || normalized.includes("good")) {
    return "celebrating";
  }
  return "listening";
}
